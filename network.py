...
def calc_pca_matrix(X_std: np.array, Y: np.array, feature_names: [str] = None, threshold: float = 0.95, show_plots: bool = False) -> np.array:
    ...
    :param Y: A numpy array containing the matching labels for the dataset.
    :param feature_names: A numpy array containing all feature names for the dataset as strings.
    :param threshold: A float representing the threshold for the variance. Default value is 0.95.
    :param show_plots: A boolean representing if the results should be plotted. Default value is False.
    :return: A numpy array representing the transformation matrix to transform datasets based on the performed PCA.
    """
    label_len = np.unique(Y).size

    if feature_names is None:
        feature_names = [str(i) for i in range(X_std.shape[1])]
    ...
    eigen_vals_feature_names = feature_names[idx]
    ...
    # Step 6: Apply the transformation matrix on X and the feature names
    X_trans = transform(X_std, transformation_matrix)
    feature_names_trans = eigen_vals_feature_names[:n_components]
    ...
    if show_plots:
        ...
        colors = plt.cm.Set1(Y / label_len)

        def plot_cum_var_exp():
            ...
            ax.annotate(f'{idx_threshold} components for {threshold:.0%} variance', xy=(idx_threshold + 5, 0.5), fontsize=12,
                        color='black')
            ax.scatter(idx_threshold, val_threshold, marker='o', s=100, facecolors='none', edgecolors='red')
            ax.tick_params(axis='x', which='minor', length=5)

        def plot_feat_vs_feat(ax, feat1, feat2):
            ax.scatter(X_std[:, feat1], X_std[:, feat2], c=colors, s=1)
            ax.set_xlabel(f'Feature {feature_names[feat1]}')
            ax.set_ylabel(f'Feature {feature_names[feat2]}')
            ax.set_title('Original data', fontsize=14)
            ax.tick_params(axis='both', which='major')

        def plot_trans_feat_vs_feat(ax, feat1, feat2):
            ax.scatter(X_trans[:, feat1], X_trans[:, feat2], c=colors, s=1)
            ax.set_xlabel(f'Feature {feature_names_trans[feat1]}')
            ax.set_ylabel(f'Feature {feature_names_trans[feat2]}')
            ax.set_title('Transformed data', fontsize=14)
            ax.tick_params(axis='both', which='major')

        def plot_eigen_vec_vs_eigen_vec(ax, eigen_vec1, eigen_vec2):
            x = eigen_vecs[:, eigen_vec1]
            y = eigen_vecs[:, eigen_vec2]
            x_norm = (x - np.min(x)) / (np.max(x) - np.min(x))
            cmap = plt.cm.jet
            colors_map = cmap(x_norm)
            ax.scatter(x, y, c=colors_map, s=10)
            ax.set_xlabel(f'Eigenvector {eigen_vec1}')
            ax.set_ylabel(f'Eigenvector {eigen_vec2}')

        # Plot cumulative explained variance ratio
        plot_cum_var_exp()

        # Plot some insights on the original data, transformed data and eigenvectors for the first three indices
        if X_std.shape[1] >= 2:
            # Create a 2x2 grid of subplots to plot features against other features
            fig, axs = plt.subplots(3, 2, figsize=(10, 10))

            plot_feat_vs_feat(axs[0, 0], feat1=0, feat2=1)
            plot_feat_vs_feat(axs[0, 1], feat1=0, feat2=2)

            plot_trans_feat_vs_feat(axs[1, 0], feat1=0, feat2=1)
            plot_trans_feat_vs_feat(axs[1, 1], feat1=0, feat2=2)

            plot_eigen_vec_vs_eigen_vec(axs[2, 0], eigen_vec1=0, eigen_vec2=1)
            plot_eigen_vec_vs_eigen_vec(axs[2, 1], eigen_vec1=0, eigen_vec2=2)

            plt.tight_layout()

    # Step 6: Return transformation matrix
...
def calc_confusion_matrix(predicted_labels: [int], ground_truth: [int], class_idx: int) -> dict:
    ...
    :return:
    """
    TP = sum(1 for i in range(len(predicted_labels)) if predicted_labels[i] == class_idx and ground_truth[i] == class_idx)
    FP = sum(1 for i in range(len(predicted_labels)) if predicted_labels[i] == class_idx and ground_truth[i] != class_idx)
    TN = sum(1 for i in range(len(predicted_labels)) if predicted_labels[i] != class_idx and ground_truth[i] != class_idx)
    FN = sum(1 for i in range(len(predicted_labels)) if predicted_labels[i] != class_idx and ground_truth[i] == class_idx)
...
def calc_f1(conf_mat: dict):
    return (2 * conf_mat["TP"]) / (2 * (conf_mat["TP"]) + conf_mat["FP"] + conf_mat["FN"])
...
class Network:
    def __init__(self, layer_shape=(), seed=-1):
        """Initializes a new feed forward neural network (FNN).

        :param layer_shape: The shape of the layers. (i.e. [700, 18, 10])
        :param seed: The seed of the network.
        """
        self.alpha = None
        ...
        self.sum_abs_thetas = dict()
        for i in range(len(self._thetas)):
            self.sum_abs_thetas[i] = []
    ...
    def train(self, hidden_layer_shape, iterations, alpha, X_train, y_train, X_val, y_val, batch_size=None,
              log_intervals=None, class_labels=(), show_plots=False, feature_names=None,
        ...
        if use_feature_scaling:
            ...
            self._transformation_matrix = calc_pca_matrix(X_std=X_train_std, Y=y_train, threshold=pca_threshold,
                                                          feature_names=self.feature_names, show_plots=show_plots)
        ...
        with tqdm(range(iterations), bar_format=progress_bar_format, postfix=f"Alpha: {postfix_alpha} ") as pbar:
            for iteration in pbar:
                ...
                postfix_alpha = f"{self.alpha:.8f}".rstrip('0').rstrip('.') if self.alpha else '0'
                pbar.set_postfix_str(f"Alpha: {postfix_alpha}, Validation Acc: {self._statistical_data['val']['acc'][-1]:.8f}", refresh=True)

        self.report_model_performance()
        self.report_confusion_matrix_2D()

        if show_plots:
            self.plot_confusion_matrix_2D_live()
            self.plot_thetas()
    ...
    def plot_thetas(self):
        vals = self.sum_abs_thetas.values()
        fig, axs = plt.subplots(nrows=1, ncols=len(vals) + 1, figsize=(6 * (len(vals) + 1), 8))

        for layer_idx, thetas in enumerate(vals):
            max_iterations = len(thetas)
            iterations = range(0, max_iterations * self.log_intervals['second'], self.log_intervals['second'])
            transposed_data = np.array(thetas).T

            ax = axs[layer_idx]
            for neuron_idx, neuron_weights in enumerate(transposed_data):
                ax.plot(iterations, neuron_weights, label=f'Neuron {neuron_idx + 1}')
            ax.set_xlabel('Iterations')
            ax.set_title(f'Hidden Layer {layer_idx + 1}: {len(transposed_data)} Neurons')
            if layer_idx == len(vals) - 1:
                ax.set_title(f'Output Layer: {len(transposed_data)} Neurons')
            if len(vals) <= 2:
                ax.set_ylabel('Absolute Sum of Thetas per Neuron')
                ax.legend(bbox_to_anchor=(1.2, 1), loc='upper center')

        self.plot_loss_and_acc(axs[len(vals)])
        fig.suptitle('Sum of Absolute Weight per Neuron per Layer')
    ...
    @classmethod
    def create_with_hyper_params(cls, source_dir):
        print("Creating network with existing hyper parameters")
        # Load hyper parameters from a given directory
        loaded_params = np.load(source_dir, allow_pickle=True)
        hyper_params_dict = dict(loaded_params.items())

        thetas = hyper_params_dict['_thetas']
        seed = hyper_params_dict['_seed']

        network = Network(layer_shape=layers_from_thetas(thetas), seed=seed)
        del hyper_params_dict['_seed']

        # Assign params back to the class hyper parameters
        for param_name, param_value in hyper_params_dict.items():
    ...
    def save(self, target_dir):
        ...
        np.savez(outfile, **hyper_params_dict)
    ...
    def calculate_CE_loss(self, prediction, ground_truth):
        ...
        :param prediction: An ndarray with shape (n_samples, n_classes) containing the raw prediction values for each sample and class.
        :param ground_truth: An ndarray with shape (n_samples, n_classes) containing the ground truth labels for each sample and class, encoded as one-hot vectors.
    ...
    def data_analysis_basic(self, dict_dir, prediction, ground_truth_oh, iteration):
        ...
        :param dict_dir: A string indicating the directory in statistical_data where the results will be stored.
    ...
    def data_analysis_advanced(self, dict_dir: str, prediction: np.array, ground_truth: np.array,
        ...
        :param dict_dir: The name of the dictionary in the statistical data where the results should be stored.
        ...
        if iteration % self.log_intervals['second'] == 0 or iteration == self.iterations - 1:
            for i, theta_arr in enumerate(self._thetas.copy()):
                theta_arr = np.array(theta_arr)
                abs_thetas = np.abs(theta_arr)
                sum_abs_theta = np.sum(abs_thetas, axis=1)
                self.sum_abs_thetas[i].append(sum_abs_theta)
    ...
    def plot_confusion_matrix_2D(self, confusion_matrix=None, labels=None):
        ...
        :param confusion_matrix: confusion matrix (list or np.array)
    ...
    def plot_confusion_matrix_2D_live(self):
